...
class Trainer() :
    def __init__(self, dataset, vocab_size, embed_size, hidden_size, bsize): 
        ...
        self.model = Model(vocab_size, embed_size, hidden_size, bsize, pre_embed=dataset.vec.embeddings)
    ...
    def train(self, n_iters, save_on_metric):
        ...
        for i in tqdm(range(n_iters)):
            loss_total_batch, loss_total = self.model.train(train_data.X, train_data.y)
            ...
            predictions_test = self.model.evaluate(test_data.X)
